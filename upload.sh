...
DevelopVersionString=`grep -E 'version.develop.*=' ModuleB_category.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' ModuleB_category.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' ModuleB_category.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' ModuleB_category.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" ModuleB_category.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' ModuleB_category.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" ModuleB_category.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' ModuleB_category.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" ModuleB_category.podspec
...
VersionLineNumber=`grep -nE 's.version.*=' ModuleB_category.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" ModuleB_category.podspec
...
pod repo push ${Repo} ModuleB_category.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
